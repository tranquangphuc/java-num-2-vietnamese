...
public class NumToViet {
            ...
            case 1:
                switch (c) {
                    ...
                    case 5:
                        return "mười lăm";
                    default:
                        return "mười " + digitsName.get(c);
                }
            default:
                switch (c) {
                    case 0:
                        return digitsName.get(b) + " mươi";
                    case 1:
                        return digitsName.get(b) + " mươi mốt";
                    case 4:
                        return digitsName.get(b) + " mươi tư";
                    case 5:
                        return digitsName.get(b) + " mươi lăm";
                    default:
                        return digitsName.get(b) + " mươi " + digitsName.get(c);
                }
        }
    }
    ...
    private String toWordsGroupByThousand(long num, List<String> thousandsName) {
        ...
        for (int i = 0; i < str.length(); i += 3) {
            groupOfThousand.add(str.substring(i, i + 3));
        }

        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < groupOfThousand.size(); i++) {
            String triple = groupOfThousand.get(i);
            String vnString = readTriple(triple, i > 0).trim();
            if (!vnString.isEmpty()) {
                sb.append(vnString)
                        .append(" ")
                        .append(thousandsName.get(groupOfThousand.size() - 1 - i))
                        .append(" ");
            }
        }
    ...
    public static void main(String[] args) {
        ...
        Boolean result = testCase.entrySet().stream().map(entry -> {
            ...
            if (!comparision) {
                System.out.printf("[Failed] result: |%s| expected |%s|%n", vietString, entry.getValue());
            } else {
                passed.getAndIncrement();
                System.out.printf("[Passed] result: |%d| - |%s|%n", entry.getKey(), entry.getValue());
            }
